    ...
    <main className=" w-full h-full">
      <div className="md:w-2/4 sm:w-3/4 m-auto p-4  my-4  rounded-lg bg-slate-800 drop-shadow-xl">
        <h2 className=" text-slate-200 text-center  text-2xl   font-extrabold font-[verdana] uppercase">
          MY FULL FULL Stack Blog App with next js
        </h2>
      </div>
      {/* Link  add*/}
        ...
        posts?.map((post:any) =>(
          <div className=" w-3/4 p-4 rounded-md mx-3  my-2 bg-slate-200 flex flex-col justify-center " key={post?.id}>
           ...
           <h2 className="mr-auto  font-semibold">{post?.title}</h2>
            ...
            {/* edit */}
            <Link href={`/blog/edit/${post.id}`} className="text-center text-xl  px-4 py-2  bg-slate-900 rounded-md  font-semibold  text-slate-200">
            Edit
            </Link>
